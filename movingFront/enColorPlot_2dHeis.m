...
Ly = 2; Lx = 11; h=5.0; v = 2; tau = 0.2; maxDim = 512;
dt = 0.1; tanhshift = 3;

R = Ly; center = ceil(Lx/2+0.5-R):floor(Lx/2+0.5+R); N = Lx*(Ly-1);
...
[tval, en, enf, enf_en0, svn, localEn, localEn_En0] = collectEData(data, Lx);
Nt = length(tval); x = (1:Lx)-Lx/2-0.5; y = 1:Ly;
...
function [tval, en, enf, enf_en0, svn, localEn, localEn_En0] = collectEData(A, Nx)
    tval = A.data(1:end,1);
    en = A.data(1:end,2);
    enf = A.data(1:end,3);
    enf_en0 = A.data(1:end,4);
    svn = A.data(1:end,5);
    localEn = A.data(1:end,   6 : Nx+5);
    localEn_En0 = A.data(1:end,  Nx+6 : end);
end
